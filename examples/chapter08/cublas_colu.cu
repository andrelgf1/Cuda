...
void generate_random_dense_matrix_column(int M, int N, float **outA)
{
    int i, j;
    double rMax = (double)RAND_MAX;
    float *A = (float *)malloc(sizeof(float) * M * N);
...
void generate_random_dense_matrix_row_major(int M, int N, float **outA)
{
    int i, j;
    double rMax = (double)RAND_MAX;
    float *A = (float *)malloc(sizeof(float) * M * N);
...
int main(int argc, char **argv)
{
    ...
    printf("\nOriginal time %f seconds\n", seconds()-start);

    start = seconds();
    generate_random_dense_matrix_row_major(M, N, &A);
    printf("Row Major time %f seconds\n", seconds()-start);

    start = seconds();
    generate_random_dense_matrix_column(M, N, &A);
    printf("Iterate over coloumn time %f seconds\n\n", seconds()-start);
