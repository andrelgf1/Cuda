...
__global__ void nestedHelloWorldDepthLimit(int const iSize, int iDepth, int max_depth)
{
    int tid = threadIdx.x;
    printf("Recursion=%d: Hello World from thread %d block %d\n ", iDepth, tid,
           blockIdx.x);
    ...
    // condition to stop recursive execution
    if (iSize == 1 ) return;
    else if (iDepth >= max_depth) {
        if(tid == 0) printf("\n Maximun depth achieved \n");
        return;
    }
    ...
    if(tid == 0 && nthreads > 0)
    {
        nestedHelloWorldDepthLimit<<<1, nthreads>>>(nthreads, ++iDepth, max_depth);
        printf("-------> nested execution depth: %d\n", iDepth);
    }
}
...
int main(int argc, char **argv)
{
    ...
    nestedHelloWorldDepthLimit<<<grid, block>>>(block.x, 0, max_d);
